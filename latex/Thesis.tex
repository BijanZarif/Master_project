...
We want to solve these equations in a unit square domain $\Omega = [0,1] x [0,1]$. We divide the boundary $\partial \Omega_F$ in 4 boundaries, as follows:

\[
\partial \Omega_F = \partial \Omega_{Top} \cup \partial \Omega_{Bottom} \cup \partial \Omega_{Cord} \cup \partial \Omega_{Tissue}
\]

On the top boundary we assume an inlet velocity $\tilde{u}$, while on the bottom boundary we assume the condition $\sigma \cdot n = 0$ (\textit{do nothing condition?}). On the left boundary, that corresponds to the spinal cord, we assume for now a no-slip boundary condition, so that the cord is rigid. Regarding the right boundary, in order to simulate an elastic property of the wall/tissue, we assume that a spring is acting on the wall. We assume that $k$ is the elastic constant, and $\vec{x}$ is the position vector. Moreover, we set equilibrium of stresses, as follows $\sigma \cdot n = \pm k \vec{x}$, where $\sigma = \nu \nabla u - pI $.
...
On $\partial \Omega_{Top}$ and $\partial \Omega_{Cord}$, I assume Dirichlet boundary conditions for the velocity $u$, hence the test function $v$ will be zero on these two boundaries. Moreover, I assumed $\sigma \cdot n = 0$ on $\partial \Omega_{Bottom}$, while $\sigma \cdot n = \pm k \vec{x} $ on $\partial \Omega_{Tissue}$, hence the boundary term becomes

\[
- \int_{\partial \Omega_F (t)} (\nu \nabla u - \frac{1}{\rho} p I) \cdot n v dx = - \int_{\partial \Omega_{Tissue}} (\pm k \vec{x}) v dx.
\]
