...
Nitsche's method was proposed as a way for treating boundary conditions in finite element method. In particular, it is used to weakly impose boundary and interface conditions, and it is applicable to a wide class of problems. Let us use Poisson's equation as a model problem. Given a domanin $\Omega$ with boundary $\Gamma = \partial \Omega$, the problem is to find $\mathbf{u}$ such that
...
where $\mathbf{u}^n$ and $\mathbf{u}^t$ are respectively the normal and tangential components of the velocity $\mathbf{u}$, and $\mathbf{u} = \mathbf{u}^n + \mathbf{u}^t$. We then multiply by a test function $\mathbf{v}$, and substitute $\mathbf{u} = \mathbf{u}^n + \mathbf{u}^t$ in~\eqref{eq:nitsche:3}, obtaining

\begin{equation}
\int_{\Omega} \nabla \mathbf{u} \cdot \nabla \mathbf{v} \, dx
- \int_{\Gamma} \frac{\partial (\mathbf{u}^n +\mathbf{u}^t)}{\partial \mathbf{n}} \, \mathbf{v} \, ds
= \int_{\Omega} \mathbf{f} \mathbf{v} \, dx. 
\end{equation}

Since we suppose that the normal and the tangential components are orthogonal 
