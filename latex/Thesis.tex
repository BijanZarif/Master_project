...
\todo[inline]{Vegard said I should choose the amplitude $A$ so it produces velocities of the order of $1-5 cm/s$. $A$ should be around $15-20kPa$.}
...
\subsection{Navier-Stokes formulation of pressure driven channel flow with $u=0$ on Tissue}
...
The space $\hat{V}$ is usually chosen such that the test function will be zero in the portion of the boundary were the solution is known. The term $\int_{\partial \Omega} \sigma \cdot \mathbf{n} \,  \mathbf{v} \, ds$ can be divided into four integrals since $\partial \Omega = \partial \Omega_{inlet} \cup \partial \Omega_{outlet} \cup \partial \Omega_{tissue} \cup \partial \Omega_{cord}$. Since on the $\partial \Omega_{cord}$ boundary we assume Dirichlet boundary condition $u=0$ for the velocity, that portion of the integral is zero, because $\mathbf{v} = 0$ on $\Omega_{cord}$. Since 

\[
\int_{\partial \Omega}  \sigma \cdot \mathbf{n} \, \mathbf{v} \, ds
= \int_{\partial \Omega_{inlet}}  \underbrace{ \sigma \cdot \mathbf{n}}_{-p_{in}} \, \mathbf{v} \, ds
+ \int_{\partial \Omega_{outlet}}  \underbrace{ \sigma \cdot \mathbf{n}}_{-p_{out}} \, \mathbf{v} \, ds
+ \int_{\partial \Omega_{tissue}}  \sigma \cdot \mathbf{n} \, \mathbf{v} \, ds
\]

we want to apply the Dirichlet boundary condition on $\partial \Omega_{tissue}$ weakly using Nitsche's method:
...
where we are weakly setting $\mathbf{u} = \mathbf{g} = \mathbf{0}$. The final variational form reads
...
+ \int_{\partial \Omega_{inlet}} p_{in} \mathbf{n} \cdot \mathbf{v} \, ds
+ \int_{\partial \Omega_{outlet}} p_{out} \mathbf{n} \cdot \mathbf{v} \, ds 
- \int_{\partial \Omega_{tissue}} \mu \frac{\partial \mathbf{u}}{\partial \mathbf{n}} \mathbf{v} \, ds \\
- \int_{\partial \Omega_{tissue}} \mu \frac{\partial \mathbf{v}}{\partial \mathbf{n}} \mathbf{(u-g)} \, ds
+ \frac{\gamma}{h} \int_{\partial \Omega_{tissue}}  \mathbf{(u-g)} \, \mathbf{v} \, ds =  \int_{\Omega} \mathbf{f} \mathbf{v} \, dx \\
\int_{\Omega}  (\nabla \cdot \mathbf{u}) q \, dx = 0.
\end{split}
\end{equation}
...
\todo[inline]{Do the same as in Stokes,  but using NS}
