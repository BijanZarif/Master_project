...
def exact_solutions(mesh, C, t):
    ...
    u_e = as_vector(( sin(2*pi*x[1])*cos(2*pi*x[0])*t, -sin(2*pi*x[0])*cos(2*pi*x[1])*t))
    #u_e = as_vector(( sin(2*pi*x[1])*cos(2*pi*x[0]), -sin(2*pi*x[0])*cos(2*pi*x[1])))
...
for dt in DT:
    ...
    for n in N:
        ...
        TH = MixedElement([V] + [P] + [V])
        VPW = FunctionSpace(mesh, TH)
    ...
    #u, p = TrialFunctions(VP)   # u is a trial function of V, while p a trial function of P
        ...
        VPW_ = Function(VPW); (u,p,w) = split(VPW_) 
        
        X = Function(V1)  # in here I will put the displacement X^(n+1) = X^n + dt*(w^n)
        ...
        w_mid = (1.0-theta)*w0 + theta*w                                    # while f is the f_exact at the time t=i+1
        ...
        while t_ < (T - 1E-9):
            ...
            solve(F == 0, VPW_, bcu + bcw, solver_parameters={"newton_solver":
        ...
        #solve(A, VP_.vector(), b)
            ...
            upw0.assign(VPW_) # the first part of VP_ is u0, and the second part is p0
            #w0.assign(project(VPW_.sub(2), V1))
            Y.vector()[:] = dt*project(VPW_.sub(2), V1).vector()[:]
