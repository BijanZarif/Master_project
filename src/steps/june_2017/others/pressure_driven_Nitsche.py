...
for n in N :
    ...
    # For Nitsche method
    nitsche_value = Constant((0.0, 0.0))
    gamma = 10.0
    h = CellSize(mesh)
    ...
    F += inner(p*n, v)*ds(3) #Nitsche terms
    F -= Constant(mu) * inner(grad(u) * n,v) * ds(3) #Nitsche terms
    F -= Constant(mu) * inner(grad(v) * n, u - nitsche_value) * ds(3) #Nitsche terms
    F -= gamma * h**-1 * inner(u - nitsche_value, v) * ds(3) #Nitsche terms
    F -= inner(f_mid,v)*dx
