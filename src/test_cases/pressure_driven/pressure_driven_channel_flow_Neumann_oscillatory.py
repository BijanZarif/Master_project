...
for n in N :
    ...
    # T = 5 
    T = 10      # only for oscillating p_inlet
    nu = 1.0/8.0
    rho = 1.0
    theta = 0.5 
    f0 = Constant((0.0, 0.0))
    ...
    #p_in  = Constant(1.0)
    p_in = Expression("a*sin(2*pi*t)", a=Constant(20.0), t=0.0, degree=2)   # only for oscillating p_inlet
    p_out = Constant(0.0) 
    ...
    F += Constant(nu) * inner(grad(u_mid), grad(v)) * dx
    F -= p * div(v) * dx
    F -= q * div(u) * dx
