...
for n in N :
    ...
    while t <= T + 1E-9:
        ...
        # Here we verified the conservation of mass
        ...
        n = FacetNormal(mesh)
        m0 = assemble(inner(u0, n) * ds(0))
        m1 = assemble(inner(u0, n) * ds(1))
        m2 = assemble(inner(u0, n) * ds(2))
        m3 = assemble(inner(u0, n) * ds(3))
        
        
        print "integral = {}".format(assemble(div(u0)*dx))
        print "sum = {}".format(m0+m1+m2+m3)
