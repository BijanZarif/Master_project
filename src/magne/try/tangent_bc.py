...
if __name__ == "__main__":
    ...
    if apply_nitsche:
        ...
        a += (- inner(grad(u)*n,t) * v_t * ds
              - inner(grad(v)*n,t) * u_t * ds
              + Constant(100) * CellSize(mesh)**-1 * inner(u_t,v_t) * ds)
        L += (- inner(grad(v)*n, t) * g * ds
              + Constant(100) * CellSize(mesh)**-1 * inner(g,v_t) * ds)
