...
\end{equation}
\\
and we can choose $Q = L^2(\Omega)$. In our setup, the boundary $\partial \Omega_t$ is divided in the four boundaries:

\begin{equation}
\partial \Omega_t= \partial \Omega_{\rm Top} \cup
\partial \Omega_{\rm Bottom} \cup
\partial \Omega_{\rm Cord} \cup
\partial \Omega_{\rm Tissue}.
\end{equation}

Since we are assuming $\Gamma_D = \partial \Omega_{Top} \cup \partial \Omega_{Cord}$, the test function $\mathbf{v}$ will be zero on these two boundaries. Moreover, we assume zero stress on the bottom boundary, i.e. $\sigma \cdot \mathbf{n} = 0$ on $\partial \Omega_{Bottom}$. The boundary term becomes
...
\begin{equation}
W = \set{\mathbf{w} \in [H^1(\Omega)]^2 | \mathbf{w}_{|_{\partial \Omega_t \setminus \Omega_{\rm Tissue}}} = \mathbf{0}, \mathbf{w}_{|_{\partial \Omega_{\rm Tissue} } }  = \mathbf{u} } 
\end{equation}

Let $\mathbf{z}$ be a test function in the space $\set{}$. We multiply equation~\eqref{eq:bc:2} by $\mathbf{z}$ and integrate on the entire domain $\Omega$ to obtain
