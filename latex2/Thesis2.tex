...
Since we are assuming $\Gamma_D = \partial \Omega_{\rm top} \cup \partial \Omega_{\rm cord}$, the test function $\mathbf{v}$ will be zero on these two boundaries. Moreover, we assume zero stress on the bottom boundary, i.e. $\sigma \cdot \mathbf{n} = 0$ on $\partial \Omega_{\rm bottom}$. The boundary term becomes
